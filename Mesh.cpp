...
void MeshColor::CreateMeshColor(GLfloat *vertices, unsigned int numOfVertices) //función de creación de VAO, VBO, IBO
{
	...
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6*sizeof(vertices[0]), 0);//Nos dice el 6*sizeof(vertices[0] cuantas columnas tenemos por renglon
	glEnableVertexAttribArray(0);
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 6 *sizeof(vertices[0]), (void*)(3*sizeof(vertices[0])));//Offset de los 3 vértices para tomar los 3 de color	Con este ignoramos 3 datos, leemos 3 datos
...
void Mesh::CreateMesh(GLfloat* vertices, unsigned int* indices, unsigned int numOfVertices, unsigned int numberOfIndices)
{
	...
	glGenBuffers(1, &IBO);		//Arreglo de índices
...
void MeshColor::RenderMeshColor() //Función de Render (Dibujado)
{
	...
	glDrawArrays(GL_TRIANGLES, 0, vertexCount);		//Para dibujar indices esta no se puede usar, Aqui el vertex count cuenta automaticamente cuando agreguemos info
	glBindVertexArray(0);
}
...
void Mesh::RenderMesh()
{
	...
	glBindVertexArray(VAO);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, IBO);
	glDrawElements(GL_TRIANGLES, indexCount, GL_UNSIGNED_INT, 0);//Se usa para dibujar indices
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
	glBindVertexArray(0);
}
