...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		model = glm::translate(model, glm::vec3(0.5f, 1.8f, 0.3f));	//Movemos la posición del objeto partiendo del origen
		...
		modelaux = model;
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		model = glm::translate(model, glm::vec3(1.2f, 1.5f, 0.3f));	//Movemos la posición del objeto partiendo del origen
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion1()), glm::vec3(0.0f, 0.0f, 1.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
