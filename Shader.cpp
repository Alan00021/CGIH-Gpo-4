...
void Shader::CreatefromString(const char* vertexCode, const char* fragmentCode)		//Lee la cadena y la compila
{
	CompileShader(vertexCode, fragmentCode);
}
void Shader::CreateFromFiles(const char* vertexLocation, const char* fragmentLocation)
{
	std::string vertexString = ReadFile(vertexLocation);		//Lee los archivos
	std::string fragmentString = ReadFile(fragmentLocation);
	const char* vertexCode = vertexString.c_str();		//Casteos para vovlerlos string
	const char* fragmentCode = fragmentString.c_str();	//Casteos para vovlerlos string
...
std::string Shader::ReadFile(const char* fileLocation)
{
	...
	std::string line = "";
	while (!fileStream.eof())	//Lee el archivo, hasta que llegue a la linea final end of file, y lo almacena en uno nuevo
	{
